...
def main():
    try:
        ...
        st.set_page_config(
            page_title="KB Article Screening Assistant",
            page_icon=":sparkles:"
        )

        st.title("KB Article Screening Assistant")
        st.subheader(
            "This AI Assistant would help you to screen available KB Articles, that are submitted to the Organization!"
        )
        ...
        if submit:
            ...
            for document_index in range(len(relevant_documents)):
                ...
                with st.expander("Show Me Summary of this KB Article ..."):
